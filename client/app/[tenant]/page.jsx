...
export default function StorefrontPage({ params }) {
  ...
  // Filtered Products (Multi-attribute filtering)
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex items-center justify-center p-4">
        <div className="text-center bg-[var(--card)] border border-[var(--card-border)] p-8 sm:p-10 rounded-3xl max-w-md w-full shadow-2xl space-y-4">
          <div className="w-14 h-14 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 flex items-center justify-center mx-auto mb-2">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          </div>
          <h1 className="font-editorial text-3xl font-semibold tracking-tight text-[var(--foreground)]">Store Not Found</h1>
          <p className="text-xs text-[var(--muted)] leading-relaxed">{error || "The store you are looking for does not exist or has been deactivated."}</p>
          <div className="pt-2">
            <a href="/" className="btn-clay text-xs inline-block">
              Back to Platform
            </a>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex flex-col font-sans transition-colors duration-300">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 py-6 w-full space-y-12">
        ...
        {/* EXPLORE OUR PRODUCTS WITH FILTER SIDEBAR */}
        ...
        {isWishlistOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex justify-end">
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="relative w-full max-w-md bg-[var(--surface)] border-l border-[var(--border)] h-full flex flex-col z-10 shadow-2xl">
              <div className="p-6 border-b border-[var(--border)] flex items-center justify-between bg-[var(--card)]">
                <div>
                  <h3 className="font-editorial text-2xl font-semibold text-[var(--foreground)]">Your Wishlist</h3>
                  <p className="text-xs text-[var(--muted)] font-mono">{wishlistCount} item{wishlistCount === 1 ? '' : 's'} saved</p>
                </div>
                <button onClick={() => setIsWishlistOpen(false)} className="p-2 text-[var(--muted)] hover:text-[var(--foreground)] rounded-full border border-[var(--border)] hover:bg-[var(--background)] transition-colors">✕</button>
              </div>
