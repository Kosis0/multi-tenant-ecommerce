...
- **Order Processing & Analytics**: Revenue metrics, total order counts, active catalog count, and interactive order status updates (`pending`, `paid`, `shipped`, `delivered`, `cancelled`).
...
This creates:
- `tenants`, `users`, `products`, `categories`, `reviews`, `wishlists`, `orders`, and `order_items` tables.
...
### Orders & Checkout API
- `POST /api/orders?tenant={slug}` — Create customer order with stock check
...
- `PATCH /api/orders/:id?tenant={slug}` — Update order status *(Protected)*
- `GET /api/admin/stats?tenant={slug}` — Store revenue & analytics *(Protected)*
